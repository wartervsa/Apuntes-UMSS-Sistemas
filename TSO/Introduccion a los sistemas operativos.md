...
### **Estructura simple**
**Sistema monolitico**
No tiene estructura clara y bien definida, todas sus componentes se cargan en conjunto en un binario como un mismo programa(el kernel), todas sus funciones se realizan en modo privilegiado. Ofrece una gran velocidad, por la "cercania", pero sacrifica seguridad e integridad, si algo cae/falla entonces todo se va abajo.
### **Estructura compuesta**
**Sistema por capas**
Es un sistema modular, los servicios del SO se dividen en capas, la capa mas bajo(la capa 0) contiene el hardware y la capa mas alta(la capa N) contiene los programas del usuario, ademas es jerarquico, la capa superior solo puede comunicarse con la capa inferior y la capa inferior solo puede brindar servicios a la superior. Una de las ventajas de esta estructura es la dificultad reducida en su menatenimiento, ya que su naturaleza modular permite debugguear directamente en la capa del problema sin afectar a las demas. Una ventaja/desventaja seria la intergracion de mas servicios/modulos, ya que se facilita la integracion de un pedazo mas al SO si tocar lo demas, pero se tiene que tener mucho cuidado en la jerarquia ya que podria afectar a logica de comunicacion entre capas. Otra desventaja seria la velocidad, debido a que se tendra que pasar por cada una de las capas y por ende tomaria mucho mas tiempo.
**MicroKernel(cliente-servidor)**
	En esta estructura aparta el kernel con unas funciones escenciales(se le denomina micronucleo) de los servicios generales, lo cuales seran ejecutados en modo usuario. El micronucleo sera la unica parte que ejecute en modo privilegiado donde no se tienen funciones concretas, ya que puede variar, pero generalmente se tiene la gestion  basica de procesos y memoria, gestion de interrupciones y servicios basico de comunicacion entre procesos. Los servicios externos son manejados como servidores(servidor de procesos, servidor de memoria, servidor de la E/S, etc.) que son solicitados por el micronucleo que recibe la orden del usuario, asi es como se convierte en un puente entre el usuario-micronucleo-servidores.
### Maquina virtual

Basicamente es lo que se aplica en las maquinas virtuales, como los servicios que ofrece VMware, que parte los recursos de un computador para 2 o mas sistemas operativos. La comunicacion va de la siguiente manera, el sistema operativo "extra" se comunica con la maquina virtual, luego esta va con el hipervisor y aqui puede ir directamente a comunicarse con el hardware o pasar antes por un sistemas operativo anfitrion. Se tiene que manejar variables como la seguridad, que no afecte un SO a otro, los recursos, que no se utilicen recursos ya asignados.
## **Sistemas por modulos**
En enfoque moderno, casi todos los computadores usan esta estructura. Aca se carga el kernel con sus funcionalidades fundamentales y lo que de la esa superioridad, para ordenadores, es la carga dinamica de funciones/servicios, solamente cuando es necesaria una funcion se la carga en memoria principal y se hace uso de ella, retirandose una vez terminada. Otra ventaja es que es caso de un error por la funcion, solo afectara a esa funcion. Trae la velocidad del sistema monolitico, centrando todo, complementadose con la modularizacion del sistema por capas, pero mas como el microkernel ya que es un uso de servicios bajo peticion. Usada por Windows actualmente, Linux, Mac OsX, Unix modernos y Solaris.